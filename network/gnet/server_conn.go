...
func (c *serverConn) Push(msg []byte) error {
	...
	return conn.AsyncWrite(msg, nil)
}
...
// 初始化连接
func (c *serverConn) init(id int64, conn gnet.Conn, cm *serverConnMgr) {
	c.id = id
	c.conn = conn
	c.connMgr = cm
...
// 检测心跳
func (c *serverConn) checkHeartbeat() {
	deadline := xtime.Now().Add(-2 * c.connMgr.server.opts.heartbeatInterval).UnixNano()
	...
	if c.connMgr.server.opts.heartbeatMechanism == TickHeartbeat {
		...
		if conn == nil {
			log.Debugf("connection is closed, cid: %d", c.id)
		}
