...
`Close()` 启动异步优雅关闭：拒绝新消息，排空已接收的发送队列，再关闭流的发送方向。
...
包括队列排空及重传时间。到期后强制关闭。`Close(true)` 立即中断底层读写，不等待
业务回调，因此可以在连接、接收和心跳回调中调用。关闭完成使用 `OnDisconnect` 观察。

`Stop()` 关闭监听器、待建流连接和活动传输，等待接入任务结束后返回。已经执行中的
业务回调可能稍后返回，因此回调中也可以调用 `Stop()`。服务器支持停止后重新启动。
...
单个有界业务队列避免双队列的额外存储及低优先级饥饿。响应式心跳通过单槽信号合并，
不会因业务队列满而阻塞读取。写入直接访问 Buffer 分段，每连接复用访问回调，不合并
复制完整业务包，也不依赖 TCP 专有的 writev。连接管理按递增 ID 分片，连接对象不复用，
避免旧引用作用于新会话。
